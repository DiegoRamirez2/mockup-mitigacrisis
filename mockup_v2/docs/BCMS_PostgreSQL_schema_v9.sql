-- ============================================================================
-- BCMS + Cumplimiento (Ley 21.663 / Multinorma) - PostgreSQL Schema v9
...
CREATE INDEX idx_org_active ON organizations(is_active) WHERE is_active = TRUE AND is_deleted = FALSE;
...
CREATE TABLE organizational_units (
  ...
  id_parent_unit         BIGINT NULL REFERENCES organizational_units(id_unit),
  code                   VARCHAR(50) NOT NULL,
  name                   VARCHAR(200) NOT NULL,
  unit_type              VARCHAR(50),
  manager_user_id        BIGINT NULL REFERENCES users(id_user),
  is_active              BOOLEAN DEFAULT TRUE,
  created_at             TIMESTAMPTZ DEFAULT now(),
  updated_at             TIMESTAMPTZ DEFAULT now(),
  CONSTRAINT uq_unit_code UNIQUE (id_organization, code)
);
CREATE INDEX idx_unit_parent ON organizational_units(id_parent_unit);
...
CREATE TABLE process_bcms_data (
  ...
  responsible_unit_id    BIGINT REFERENCES organizational_units(id_unit),
  created_at             TIMESTAMPTZ DEFAULT now(),
  updated_at             TIMESTAMPTZ DEFAULT now(),
...
CREATE TABLE applied_controls (
  ...
  description            TEXT,
  control_type           VARCHAR(50),
  control_scope          VARCHAR(30) NOT NULL,
  target_macroprocess_id INT NULL REFERENCES macroprocesses(id_macroprocess),
  target_process_id      INT NULL REFERENCES processes(id_process),
  target_subprocess_id   INT NULL REFERENCES subprocesses(id_subprocess),
  target_procedure_id    INT NULL REFERENCES procedures(id_procedure),
  ...
  responsible_unit_id    BIGINT NULL REFERENCES organizational_units(id_unit),
  linked_requirements    JSONB,
  created_at             TIMESTAMPTZ DEFAULT now(),
  updated_at             TIMESTAMPTZ DEFAULT now(),
  CONSTRAINT ck_control_scope CHECK (control_scope IN ('MACROPROCESS', 'PROCESS', 'SUBPROCESS', 'PROCEDURE', 'GLOBAL')),
  ...
  CONSTRAINT ck_control_target CHECK (
    (control_scope = 'MACROPROCESS' AND target_macroprocess_id IS NOT NULL AND target_process_id IS NULL AND target_subprocess_id IS NULL AND target_procedure_id IS NULL) OR
    (control_scope = 'PROCESS' AND target_process_id IS NOT NULL AND target_macroprocess_id IS NULL AND target_subprocess_id IS NULL AND target_procedure_id IS NULL) OR
    (control_scope = 'SUBPROCESS' AND target_subprocess_id IS NOT NULL AND target_macroprocess_id IS NULL AND target_process_id IS NULL AND target_procedure_id IS NULL) OR
    (control_scope = 'PROCEDURE' AND target_procedure_id IS NOT NULL AND target_macroprocess_id IS NULL AND target_process_id IS NULL AND target_subprocess_id IS NULL) OR
    (control_scope = 'GLOBAL' AND target_macroprocess_id IS NULL AND target_process_id IS NULL AND target_subprocess_id IS NULL AND target_procedure_id IS NULL)
  )
);

CREATE INDEX idx_control_scope ON applied_controls(control_scope);
CREATE INDEX idx_control_macro ON applied_controls(target_macroprocess_id) WHERE target_macroprocess_id IS NOT NULL;
CREATE INDEX idx_control_proc ON applied_controls(target_process_id) WHERE target_process_id IS NOT NULL;
CREATE INDEX idx_control_subproc ON applied_controls(target_subprocess_id) WHERE target_subprocess_id IS NOT NULL;
CREATE INDEX idx_control_procedure ON applied_controls(target_procedure_id) WHERE target_procedure_id IS NOT NULL;
...
CREATE TABLE risks (
  ...
  CONSTRAINT ck_risk_target CHECK (
    (risk_scope = 'MACROPROCESS' AND target_macroprocess_id IS NOT NULL) OR
    (risk_scope = 'PROCESS' AND target_process_id IS NOT NULL) OR
    (risk_scope = 'SUBPROCESS' AND target_subprocess_id IS NOT NULL) OR
    (risk_scope = 'PROCEDURE' AND target_procedure_id IS NOT NULL) OR
    (risk_scope = 'GLOBAL')
  )
);
CREATE INDEX idx_risk_domain ON risks(risk_domain);
CREATE INDEX idx_risk_scope ON risks(risk_scope);
CREATE INDEX idx_risk_proc ON risks(target_process_id) WHERE target_process_id IS NOT NULL;
...
CREATE INDEX idx_bia_proc ON bia_assessments(target_process_id) WHERE target_process_id IS NOT NULL;
...
CREATE INDEX idx_tag_active ON tags(is_active) WHERE is_active = TRUE;
...
GROUP BY t.id_threat, t.threat_code, t.name, lv_cat.label, lv_src.label, t.is_active;
